                ...
                <div class="row form-group" wire:ignore>
                    ...
                    <div class="col-sm-8">
                        <select name="tags" id="select-tag" label="برچسب ها" multiple="multiple">
                            ...
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}" @selected(in_array($tag->id, $form->tags->pluck('id')->toArray()))>{{ $tag->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div id="attribute-box-{{ $attribute->id }}" wire:key="{{ $attribute->id }}">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                ...
                                {{-- if this attributes is among this product attributes, set the input value with pivot table value --}}
                                value="{{ $form->attributes->contains($attribute) ? $form->attributes->where('id', $attribute->id)->first()->pivot->value : '' }}">
                            </div>
                        </div>
                    </div>
                @endforeach
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                ...
                $('#summernote2').summernote('code', @this.form.long_description);
